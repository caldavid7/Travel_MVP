...
const ImageSlider = ({ response, isLoaded }: Props) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            paginate(1); // Move to the next image
        }, 5000); // 5000ms = 5 seconds
    
        return () => clearInterval(interval); // Clear the interval on component unmount
    }, [page, paginate]); // Re-run this effect whenever the page changes
    ...
    return (
        <div className='relative row-start-1 lg:row-start-auto overflow-hidden'>
            ...
            <div className='absolute inset-0 bg-darken_bottom z-0 pointer-events-none'></div>
            <div className='absolute grid place-items-center gap-4 left-0 bottom-10 w-full '>
